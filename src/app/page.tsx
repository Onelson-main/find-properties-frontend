  ...
  return (
    <div id='base'>
      ...
      {loading ? <p className='text-center'>Loading..</p>
        : <>
          {properties.length > 0
            ? <div className='flex flex-col md:flex-row md:justify-between md:flex-wrap gap-4'>
              {properties.map((p) => {
                return (
                  <div key={p.id} className='bg-BACKGROUND text-FOREGROUND overflow-hidden rounded-md mb-1 border border-FOREGROUND flex flex-row md:flex-col md:w-72 items-stretch'>
                    <img src={p.imageUrl} alt="" className='w-20 sm:w-32 md:w-72' />
                    <div className='flex flex-1 flex-col justify-between text-left p-2 py-1'>
                      ...
                      <div className='flex flex-row justify-between'>
                        ...
                        <p className='block sm:hidden md:block font-bold text-right'>{p.price}</p>
                      </div>
                    </div>
                    <p className='hidden sm:block md:hidden ml-auto p-2 self-center text-2xl font-bold text-right'>{p.price}</p>
                  </div>)
              })}</div>
